...
class Database {
  ...
  addMessage(
    ...
    const atTargetUserBlockList = this.getBlockList(atTarget);
    if (
      atTarget &&
      atTargetUserBlockList.length !== 0 &&
      atTargetUserBlockList.indexOf(user) !== -1
    ) {
      isBlocked = true;
      return isBlocked;
    }

    const dmTargetUserBlockList = this.getBlockList(dmTarget);
    if (
      dmTarget &&
      dmTargetUserBlockList.length !== 0 &&
      dmTargetUserBlockList.indexOf(user) !== -1
    ) {
      isBlocked = true;
      return isBlocked;
    }
    ...
    this.messages.unshift(
      new Message(message, user, this.messageCount++, atTarget, dmTarget)
    );
    return isBlocked;
  }
  ...
  getMessages(pagingToken: string, user: string): MessagesContainer {
    ...
    if (pagingToken !== "") {
      ...
      nextMessageIndex = this.messages.findIndex(
    ...
    while (
      addedCounter < 20 &&
      checkedCounter < this.messages.length - nextMessageIndex
    ) {
      ...
      if (
        ...
        if (message.user !== user) {
          ...
          const blockList = this.getBlockList(user);
          if (blockList.indexOf(message.dmTarget) === -1) {
            messagesToSend.push(message);
            addedCounter++;
          }
        }
